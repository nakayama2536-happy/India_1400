...
判断画面では、GPTへ追加分析を渡すタイミングだけを4系統で判定します。

- 売買条件接近：次の3分割条件が成立直前または成立
- 相場急変：NIFTY、USD/INR、Brent、India VIX、FII/FPI等の大きな変化
- 判断矛盾：短期/中期予測、価格/Breadth、テクニカル/資金フロー等の不一致
- データ品質：主要データ欠損、重大QC異常

表示は「深掘り不要 / 深掘り推奨 / 深掘り優先」の3段階です。これは売買シグナルではありません。深掘りMarkdownには `trigger_context` と `mandatory_checks` を追加し、その日のトリガーに応じた重点確認事項をGPTへ明示します。休場・時間外であることだけでは深掘り警告を立てません。
